...
#
# Rationale:
#   - Cercospora is the primary disease of interest; catch it first.
...
def predict_with_thresholds(scores_array, cercospora_thresh, healthy_thresh):
    """
    ...
    for scores in scores_array:
        ...
        elif scores[HEALTHY_IDX] >= healthy_thresh:
            preds.append(HEALTHY_IDX)
        else:
            preds.append(OD_IDX)   # last resort
    return np.array(preds)


best_macro_f1          = 0.0
best_cercospora_thresh = MIN_CERCOSPORA_THRESH
best_healthy_thresh    = MIN_HEALTHY_THRESH

# Coarse 2D grid search (step=0.05) — floors enforced by MIN_* constants
print("Running coarse 2D grid search...")
for c_t in np.arange(MIN_CERCOSPORA_THRESH, 0.85, 0.05):
    for h_t in np.arange(MIN_HEALTHY_THRESH, 0.85, 0.05):
        preds    = predict_with_thresholds(val_y_score, c_t, h_t)
        macro_f1 = f1_score(val_y_true, preds, average='macro', zero_division=0)
        if macro_f1 > best_macro_f1:
            best_macro_f1          = macro_f1
            best_cercospora_thresh = c_t
            best_healthy_thresh    = h_t

# Fine search ±0.06 around best coarse point (step=0.01) — floors still enforced
print("Fine-tuning around best coarse point...")
for c_t in np.arange(max(MIN_CERCOSPORA_THRESH, best_cercospora_thresh - 0.06),
                      best_cercospora_thresh + 0.07, 0.01):
    for h_t in np.arange(max(MIN_HEALTHY_THRESH, best_healthy_thresh - 0.06),
                          best_healthy_thresh + 0.07, 0.01):
        preds    = predict_with_thresholds(val_y_score, c_t, h_t)
        macro_f1 = f1_score(val_y_true, preds, average='macro', zero_division=0)
        if macro_f1 > best_macro_f1:
            best_macro_f1          = macro_f1
...
print(f"  Cercospora threshold: {best_cercospora_thresh:.2f}  (min floor: {MIN_CERCOSPORA_THRESH})")
print(f"  Healthy threshold:    {best_healthy_thresh:.2f}  (min floor: {MIN_HEALTHY_THRESH})")

# Per-class recall/precision on val set with optimized thresholds
opt_val_preds = predict_with_thresholds(val_y_score, best_cercospora_thresh, best_healthy_thresh)
...
threshold_config = {
    ...
    "healthy_threshold":      round(float(best_healthy_thresh), 2),
    ...
    "note": (
        ...
        "Elif healthy score >= healthy_threshold → healthy. "
        "Else → other_diseases (last resort — model is not confident in either primary class)."
    )
}
...
print(f"\n--- Threshold-Optimized (Cercospora={best_cercospora_thresh:.2f}, Healthy={best_healthy_thresh:.2f}) ---")
...
print(f"Cercospora threshold: {best_cercospora_thresh:.2f}  (min floor: {MIN_CERCOSPORA_THRESH})")
print(f"Healthy threshold:    {best_healthy_thresh:.2f}  (min floor: {MIN_HEALTHY_THRESH})")
print(f"other_diseases:       last resort (predicted when neither threshold is met)")
print("-"*70)
